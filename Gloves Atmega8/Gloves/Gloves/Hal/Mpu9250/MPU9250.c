...
//   _delay_ms(1);
//   
//   M_Twi_SendSlaveAddressWrite(address);
//   _delay_ms(1);
//   
//   M_Twi_SendByte(MPU9250_ADDR_PWR_MGMT_1);
//   _delay_ms(1);
//   M_Twi_SendByte(0x00);
//   _delay_ms(1);
//   M_Twi_StopCondition();
  ...
  //_delay_ms(1);
  M_Twi_SendSlaveAddressWrite(address);
//   _delay_ms(1);M_Dio_PinToggle(_PC0);_delay_ms(100); // off
  M_Twi_SendByte(MPU9250_ADDR_ACCELCONFIG);
  //
  M_Twi_SendByte(mode);
  //_delay_ms(1);
  M_Twi_StopCondition();
...
//	i2c_readByte(address,0x40,&accelBuf[5]);	
	...
	M_Twi_SendSlaveAddressRead(address);
	accelBuf[0] = M_Twi_ReadByte(ACK);//_delay_ms(1);
	accelBuf[1] = M_Twi_ReadByte(ACK);//_delay_ms(1);
	accelBuf[2] = M_Twi_ReadByte(ACK);//_delay_ms(1);
	accelBuf[3] = M_Twi_ReadByte(ACK);//_delay_ms(1);
	accelBuf[4] = M_Twi_ReadByte(ACK);//_delay_ms(1);
	accelBuf[5] = M_Twi_ReadByte(NACK);//_delay_ms(1);
	M_Twi_StopCondition();
	return;
